...
public class XiuxianController extends BaseController {
    ...
    public String xiuxian(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if(secondMenuId!=""){
        	...
        	thirdShowName="全部";
        	dbKit = new DbKit("select * from security_news where  review_status=1 and menu_id in (select id from security_menu where  parent_id=1058) order by orderlist asc");
         }
