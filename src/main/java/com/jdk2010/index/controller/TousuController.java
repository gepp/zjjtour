...
public class TousuController extends BaseController {
	...
	public String tousu(HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		DbKit dbKit = new DbKit(
				"select * from member_complain where replay_status=1 and review_status=1  order by complain_time desc");
	...
	public String wantTousu(HttpServletRequest request,
	...
	public String tousuDetail(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
	...
	public void saveTousu(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
	...
	public String myTousuList(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		Map<String, Object> indexMap = dalClient
				.queryForObject("select * from system_indexsetting");
		setAttr("indexMap", indexMap);
		...
		if (member == null) {
			...
			if (complain_title != null && !"".equals(complain_title)) {
				searchSQL = searchSQL + " and  complain_title LIKE '%"
						+ complain_title + "%'";
