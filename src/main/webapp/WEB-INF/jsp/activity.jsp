	...
	<body>
		...
		<!--面包屑  end-->
		 <div class="list-label">
			...
			<button <c:if test="${activityStatus=='' }">class="active"</c:if>  onclick="jumpActivity('')">全部</button>
			<button <c:if test="${activityStatus=='0' }">class="active"</c:if> onclick="jumpActivity('0')">未开始</button>
			<button <c:if test="${activityStatus=='1' }">class="active"</c:if>onclick="jumpActivity('1')">进行中</button>
			<button <c:if test="${activityStatus=='2' }">class="active"</c:if>onclick="jumpActivity('2')">已结束</button>
		</div>
<!--图片列表-->
								...
								<ul class="clr">
									...
									<li class="transition" onclick="jumpDetail('${item.id}')">
										<div class="img-content"><img src="${item.smallimg }" class="transition">
											...
											<span>活动介绍：${item.abstractContent }</span></div>
											<div class="intro">
